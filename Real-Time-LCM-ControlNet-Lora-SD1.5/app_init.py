...
def init_app(app: FastAPI, user_data: UserData, args: Args, pipeline):
        ...
        try:
            ...
            async def generate():
                ...
                while True:
                    ...
                    last_params = params
                    image = pipeline.predict(params,user_id)
                    if image is None:
    ...
    @app.get('/get_dynamic_filename')
    def get_dynamic_filename():
        dynamic_filename = pipeline.get_dynamic_filename()

        # dynamic_filename = output_filename
        return JSONResponse({"dynamic_filename": dynamic_filename})

    # if __name__ == '__main__':
    #     app.run(debug=True)

    app.mount("/", StaticFiles(directory="public", html=True), name="public")
    # app.mount("public/download", StaticFiles(directory="download"), name="download")
